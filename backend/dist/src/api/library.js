...
async function getOrCreateLibrarySetting(schoolId) {
    let setting = await prisma_1.default.librarySetting.findUnique({ where: { schoolId } });
    if (!setting) {
        setting = await prisma_1.default.librarySetting.create({
            data: {
                schoolId,
...
router.patch('/settings', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'LIBRARIAN', 'BURSAR'), async (req, res) => {
    try {
        ...
        const data = {};
        if (defaultLoanPeriodDays !== undefined)
            data.defaultLoanPeriodDays = parseInt(defaultLoanPeriodDays);
        if (studentDailyFine !== undefined)
            data.studentDailyFine = parseFloat(studentDailyFine);
        if (studentMaxFine !== undefined)
            data.studentMaxFine = parseFloat(studentMaxFine);
        if (staffDailyFine !== undefined)
            data.staffDailyFine = parseFloat(staffDailyFine);
        if (staffMaxFine !== undefined)
            data.staffMaxFine = parseFloat(staffMaxFine);
        if (accrueOnWeekends !== undefined)
            data.accrueOnWeekends = Boolean(accrueOnWeekends);
        if (studentMaxLoans !== undefined)
            data.studentMaxLoans = parseInt(studentMaxLoans);
        if (staffMaxLoans !== undefined)
            data.staffMaxLoans = parseInt(staffMaxLoans);
        if (maxCopiesSameTitle !== undefined)
            data.maxCopiesSameTitle = parseInt(maxCopiesSameTitle);
        if (blockThresholdFine !== undefined)
            data.blockThresholdFine = parseFloat(blockThresholdFine);
        const setting = await prisma_1.default.librarySetting.upsert({
            where: { schoolId },
    ...
    try {
        ...
        let candidates = books.filter(book => {
            ...
            if (yearFilter && yearFilter !== 'all') {
                ...
                if (bookYear !== yearFilter)
                    return false;
            }
            if (languageFilter && languageFilter !== 'all') {
                const lang = (book.language || 'English').toLowerCase();
                if (lang !== languageFilter.toLowerCase())
                    return false;
            }
    ...
    try {
        ...
        if (Array.isArray(authors)) {
            ...
            try {
                ...
                authorList = authors.split(',').map((s) => s.trim()).filter(Boolean);
            }
        }
        ...
        if (bookPrice && bookPrice > 0 && source !== 'Donated') {
            ...
            try {
                const [assetId, cashId] = await Promise.all([
                    (0, coa_seeder_1.getAccountId)(schoolId, '2100', prisma_1.default),
...
router.patch('/books/:id', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'ANCILLARY', 'TEACHER', 'LIBRARIAN'), upload_1.libraryUpload.fields([
    ...
    try {
        ...
        if (shelfLocation !== undefined)
            data.shelfLocation = shelfLocation;
...
router.patch('/categories/:id', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'LIBRARIAN'), async (req, res) => {
    ...
    const { name } = req.body;
    try {
        const category = await prisma_1.default.libraryCategory.update({
            where: { id },
            data: { name: name.trim() }
        });
        res.json(category);
...
router.get('/borrowers/validate', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        for (const loan of activeLoans) {
            const { fineAmount } = (0, library_reminder_job_1.computeLoanFine)(loan, setting);
...
router.post('/loans/issue', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'ANCILLARY', 'TEACHER', 'LIBRARIAN'), async (req, res) => {
    ...
    try {
        ...
        if (book.available <= 0)
            return res.status(400).json({ error: `"${book.title}" has 0 copies available for issue` });
        ...
        const updatedBook = await prisma_1.default.book.findUnique({ where: { id: bookId } });
        // Fetch borrower name for confirmation toast
        let borrowerName = 'Borrower';
        if (isStudent) {
            const stu = await prisma_1.default.student.findUnique({ where: { id: studentId } });
            if (stu)
                borrowerName = stu.name;
        }
        else if (userId) {
...
router.get('/loans', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const studentActiveCounts = await prisma_1.default.bookLoan.groupBy({
            by: ['studentId'],
            where: { schoolId, status: 'borrowed', studentId: { not: null } },
            _count: { id: true }
        });
        const staffActiveCounts = await prisma_1.default.bookLoan.groupBy({
            by: ['userId'],
...
router.post('/reservations', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (req.user.role === 'STUDENT') {
            const s = await prisma_1.default.student.findFirst({ where: { userId: req.user.id } });
            if (s)
        ...
        const reservation = await prisma_1.default.bookReservation.create({
            data: {
                schoolId,
                bookId,
...
router.patch('/reservations/:id/status', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'LIBRARIAN'), async (req, res) => {
    ...
    const schoolId = req.user.schoolId;
    try {
        const reservation = await prisma_1.default.bookReservation.findFirst({
            where: { id, schoolId },
            include: {
                book: true,
                student: { include: { user: true } },
        ...
        const updated = await prisma_1.default.bookReservation.update({
            where: { id },
            data: {
                status: newStatus,
                readyAt,
...
router.get('/digital-resources', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const resources = await prisma_1.default.libraryDigitalResource.findMany({
            where: {
                schoolId,
        ...
        const checked = await Promise.all(resources.map(async (r) => {
            if (r.expiryDate && new Date(r.expiryDate) < now && r.status === 'Active') {
                await prisma_1.default.libraryDigitalResource.update({
                    where: { id: r.id },
...
router.post('/digital-resources', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'LIBRARIAN', 'TEACHER'), upload_1.libraryUpload.single('file'), async (req, res) => {
    try {
        ...
        let fileSize = null;
        if (file) {
            fileUrl = path_1.default.join(req.uploadCategoryPath || '', file.filename).replace(/\\/g, '/');
        ...
        const digital = await prisma_1.default.libraryDigitalResource.create({
            data: {
                schoolId,
                title,
...
router.get('/my-books', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const formattedLoans = await Promise.all(loans.map(async (l) => {
            ...
            const { daysOverdue, fineAmount, isOverdue } = (0, library_reminder_job_1.computeLoanFine)(l, setting);
            return {
                ...l,
                daysOverdue,
                fineAmount,
...
router.get('/reports', auth_1.requireAuth, async (req, res) => {
    ...
    if (dateRange === 'today') {
        ...
        filterStart = new Date(req.query.startDate);
        filterEnd = new Date(req.query.endDate);
    ...
    try {
        ...
        const [allSchoolAssets, allBooks, allCategories, allLoans, allReservations, allStudents, allClasses, allRequisitions] = await Promise.all([
            ...
            // Book reservations
            prisma_1.default.bookReservation.findMany({
                where: { schoolId },
                include: {
            ...
            // School Classes
            prisma_1.default.schoolClass.findMany({
                where: { schoolId },
                select: { id: true, name: true }
            }),
            // School Requisitions (reusing main system requisitions table)
            prisma_1.default.requisition.findMany({
                where: { schoolId },
        ...
        allLoans.forEach(l => {
            ...
            const h = d.getHours();
            if (h >= 8 && h < 10)
                hourSlots['08:00 - 10:00']++;
            else if (h >= 10 && h < 12)
                hourSlots['10:00 - 12:00']++;
            else if (h >= 12 && h < 14)
                hourSlots['12:00 - 14:00']++;
            else if (h >= 14 && h < 16)
                hourSlots['14:00 - 16:00']++;
            else if (h >= 16)
                hourSlots['16:00 - 18:00']++;
        });
