...
const isChaplaincyAdmin = (user) => {
    ...
    return user.role === 'SCHOOL_ADMIN' ||
        user.secondaryRoles.some((role) => chaplaincyRoles.includes(role));
};
/**
 ...
 */
router.get('/latest-reflection', async (req, res) => {
    try {
        const reflection = await prisma_1.default.announcement.findFirst({
            where: {
                schoolId: req.user.schoolId,
                title: 'Daily Reflection',
            },
