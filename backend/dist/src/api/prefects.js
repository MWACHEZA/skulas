...
router.patch('/reports/:id/status', async (req, res) => {
    ...
    try {
        const report = await prisma_1.default.prefectReport.update({
            where: { id: id },
            data: { punishmentStatus }
        });
        await (0, audit_1.logAction)(req, 'UPDATE_CONDUCT_REPORT_STATUS', 'PrefectReport', report.id, { punishmentStatus });
        res.json(report);
    }
    catch (error) {
