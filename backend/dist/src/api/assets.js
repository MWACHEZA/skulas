...
router.get('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const rawAssets = await prisma_1.default.asset.findMany({
            where: whereClause,
            include: {
        ...
        if (searchQuery) {
            filtered = rawAssets.filter(a => a.name.toLowerCase().includes(searchQuery) ||
 ...
 */
router.get('/users-search', auth_1.requireAuth, async (req, res) => {
    const schoolId = req.user.schoolId;
...
router.post('/', auth_1.requireAuth, upload_1.assetUpload.array('attachments', 5), async (req, res) => {
    ...
    const { name, category, quantity = 1, location, department: requestedDepartment, condition = 'good', serialNumber, 
    // Admin / Bursar extra fields
    supplierName, purchaseDate, purchasePrice, invoiceNumber, depreciationRate, warrantyExpiry, custodianId, nextMaintenance, maintenanceInterval, photoUrl } = req.body;
    if (!name || !category) {
    ...
    try {
        ...
        if (role === 'LIBRARIAN')
            ...
            assignedDepartment = 'Clinic';
        else if (role === 'ANCILLARY')
            assignedDepartment = 'Ancillary';
        else if (isFinRole && requestedDepartment)
            assignedDepartment = requestedDepartment;
        // Approval workflow:
        ...
        const totalExistingAssets = await prisma_1.default.asset.count({ where: { schoolId } });
        const createdAssets = await prisma_1.default.$transaction(async (tx) => {
            const results = [];
 ...
 */
router.post('/:id/approve-hod', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    const role = (req.user.role || '').toUpperCase();
    const schoolId = req.user.schoolId;
    try {
 ...
 */
router.post('/:id/approve-bursar', auth_1.requireAuth, (0, auth_1.requireRole)('BURSAR', 'SCHOOL_ADMIN', 'SUPER_ADMIN'), async (req, res) => {
    const { id } = req.params;
    const schoolId = req.user.schoolId;
    try {
        const asset = await prisma_1.default.asset.findFirst({ where: { id: String(id), schoolId } });
 ...
 */
router.post('/:id/approve-admin', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'SUPER_ADMIN'), async (req, res) => {
    const { id } = req.params;
    const schoolId = req.user.schoolId;
    try {
        const asset = await prisma_1.default.asset.findFirst({ where: { id: String(id), schoolId } });
 ...
 */
router.post('/:id/reject', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'BURSAR', 'SUPER_ADMIN', 'TEACHER'), async (req, res) => {
    const { id } = req.params;
...
router.delete('/:id', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        await prisma_1.default.$transaction([
            prisma_1.default.assetIncident.deleteMany({ where: { assetId: String(id) } }),
            prisma_1.default.assetMaintenance.deleteMany({ where: { assetId: String(id) } }),
            prisma_1.default.asset.delete({ where: { id: String(id), schoolId } })
        ]);
 ...
 */
router.post('/:id/transfer', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'BURSAR', 'LIBRARIAN', 'SUPER_ADMIN'), async (req, res) => {
    const { id } = req.params;
 ...
 */
router.post('/:id/dispose', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'BURSAR', 'SUPER_ADMIN'), async (req, res) => {
    const { id } = req.params;
 ...
 */
router.post('/:id/report-damage', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        const [incident, updatedAsset] = await prisma_1.default.$transaction([
            prisma_1.default.assetIncident.create({
                data: {
                    assetId: String(id),
 ...
 */
router.patch('/:id', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'BURSAR', 'SUPER_ADMIN'), upload_1.assetUpload.array('attachments', 5), async (req, res) => {
    const { id } = req.params;
    const role = (req.user.role || '').toUpperCase();
    const isFinRole = isFinancialRole(role);
    const files = req.files || [];
    try {
        const existing = await prisma_1.default.asset.findFirst({ where: { id: String(id), schoolId: req.user.schoolId } });
        if (!existing)
            return res.status(404).json({ error: 'Asset not found' });
        ...
        const { name, category, serialNumber, location, department, condition, status, custodianId, purchaseDate, purchasePrice, supplierName, invoiceNumber, depreciationRate, warrantyExpiry, nextMaintenance, maintenanceInterval } = req.body;
        const data = {};
        if (name)
            data.name = name;
        if (category)
        ...
        if (custodianId !== undefined)
            data.custodianId = custodianId ? String(custodianId) : null;
        ...
        if (maintenanceInterval !== undefined)
            data.maintenanceInterval = maintenanceInterval ? parseInt(maintenanceInterval) : null;
        if (isFinRole) {
            ...
            if (depreciationRate !== undefined)
                data.depreciationRate = depreciationRate ? parseFloat(depreciationRate) : null;
 ...
 */
router.patch('/incident/:id/resolve', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'SUPER_ADMIN'), async (req, res) => {
    const { id } = req.params;
 ...
 */
router.post('/maintenance/perform', auth_1.requireAuth, (0, auth_1.requireRole)('SCHOOL_ADMIN', 'SUPER_ADMIN'), async (req, res) => {
    const { assetId, description, cost, performedDate } = req.body;
    const pDate = performedDate ? new Date(performedDate) : new Date();
    try {
        const asset = await prisma_1.default.asset.findFirst({ where: { id: String(assetId), schoolId: req.user.schoolId } });
        if (!asset)
            return res.status(404).json({ error: 'Asset not found' });
