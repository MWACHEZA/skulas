...
router.get('/schools/:code/data', async (req, res) => {
    ...
    try {
        ...
        if (!school)
            return res.status(404).json({ error: 'School not found' });
        const customContentObj = (school.customContent || {});
...
router.post('/applications', async (req, res) => {
    try {
        const { schoolCode, vacancyId, applicantName, gender, email, phone, qualification, skills, workExperience, address, coverLetter, resumeUrl, photoUrl } = req.body;
