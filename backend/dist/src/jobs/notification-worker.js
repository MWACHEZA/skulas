...
class NotificationWorker {
    ...
    async processQueue() {
        ...
        const batch = await prisma_1.default.notificationQueue.findMany({
            where: {
                status: 'PENDING',
        ...
        for (const job of batch) {
            ...
            try {
                ...
                if (job.type === 'WhatsApp') {
                    ...
                    const settings = await prisma_1.default.schoolSetting.findFirst({
                    ...
                    try {
                        const parent = await prisma_1.default.parent.findFirst({ where: { phone: job.recipientPhone } });
                        if (parent?.preferredLanguage) {
                            preferredLanguage = parent.preferredLanguage;
                        }
                        else {
                            const student = await prisma_1.default.student.findFirst({ where: { phone: job.recipientPhone } });
                            if (student?.preferredLanguage) {
                                preferredLanguage = student.preferredLanguage;
                            }
                            else {
                                const user = await prisma_1.default.user.findFirst({ where: { phone: job.recipientPhone } });
