 ...
 * 0-49    | Fail        | 0.0
 */
export function calculateUniversityDivision(score: number, level: ProgramLevel = 'UNDERGRADUATE'): string {
...
export function calculateGradePoint(score: number, level: ProgramLevel = 'UNDERGRADUATE'): number {
  if (level === 'UNDERGRADUATE') {
...
export function calculateGPA(grades: { score: number; credits?: number }[], level: ProgramLevel = 'UNDERGRADUATE'): number {
...
export function determineAcademicStanding(results: { 
  score: number; 
  isPassed: boolean;
}[], previousFailsCount: number = 0): { 
  standing: AcademicStanding; 
  reasons: string[];
} {
  const totalModules = results.length;
  if (totalModules === 0) return { standing: 'Normal', reasons: [] };
 ...
 * 9: PhD
 */
export function generateCourseCode(deptCode: string, part: number, sequence: number, level: ProgramLevel = 'UNDERGRADUATE'): string {
...
export interface AdmissionCriteria {
  oLevels: string[]; // List of subject names
  aLevels: string[]; // List of subject names
  age?: number;
  entryCategory: 'Normal' | 'Special' | 'Mature';
}

export function checkUniversityEligibility(criteria: AdmissionCriteria): { 
  eligible: boolean; 
  reasons: string[];
  flags: string[];
} {
  ...
  const getUniqueCount = (subjects: string[]) => {
  ...
  if (criteria.entryCategory === 'Normal') {
    if (uniqueOLevels < 5) reasons.push(`Requires at least 5 O-Level subjects (Unique: ${uniqueOLevels})`);
    if (!hasEng) reasons.push('Missing English Language at O-Level');
    if (!hasMath) reasons.push('Missing Mathematics at O-Level');
    if (criteria.aLevels.length < 2) reasons.push(`Requires at least 2 A-Level subjects (Current: ${criteria.aLevels.length})`);
  }

  if (criteria.entryCategory === 'Mature') {
    if (!criteria.age || criteria.age < 25) reasons.push(`Mature entry requires age >= 25 (Current: ${criteria.age || 'Unknown'})`);
    if (uniqueOLevels < 5) reasons.push(`Requires at least 5 O-Level subjects (Unique: ${uniqueOLevels})`);
    if (!hasEng) reasons.push('Missing English Language at O-Level');
    if (!hasMath) reasons.push('Missing Mathematics at O-Level');
  }
