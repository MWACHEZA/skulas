...
router.post('/visitors', async (req: AuthRequest, res) => {
  try {
...
// Delete endpoints
router.delete('/inquiries/:id', async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const record = await prisma.admissionInquiry.findFirst({
      where: { id, schoolId: req.user!.schoolId as string }
    });
    if (!record) return res.status(404).json({ error: 'Inquiry not found' });
    await prisma.admissionInquiry.delete({
      where: { id }
    });
...
router.delete('/visitors/:id', async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const record = await prisma.visitorLog.findFirst({
      where: { id, schoolId: req.user!.schoolId as string }
    });
    if (!record) return res.status(404).json({ error: 'Visitor log not found' });
...
router.delete('/calls/:id', async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const record = await prisma.phoneCallLog.findFirst({
      where: { id, schoolId: req.user!.schoolId as string }
    });
    if (!record) return res.status(404).json({ error: 'Call log not found' });
...
router.delete('/complaints/:id', async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const record = await prisma.frontOfficeComplaint.findFirst({
      where: { id, schoolId: req.user!.schoolId as string }
    });
    if (!record) return res.status(404).json({ error: 'Complaint not found' });
    await prisma.frontOfficeComplaint.delete({
      where: { id }
    });
    res.json({ success: true });
  } catch (error) {
