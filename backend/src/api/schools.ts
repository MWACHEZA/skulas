...
import { SystemSettingsSchema, UpdatePlanSchema, UpdateBrandingSchema, UpdateSchoolInfoSchema, HouseSchema, AssignHouseStudentSchema, ClubSchema, SportSchema, SportEquipmentSchema, HolidaySchema, SupplierCategoriesSchema, SuperAdminSchoolSchema } from '../schemas/school.schema';
 ...
 */
router.patch('/me/plan', requireAuth, requireRole('SCHOOL_ADMIN'), validate(UpdatePlanSchema), async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.patch('/branding', requireAuth, requireRole('SCHOOL_ADMIN'), upload.single('logo'), validate(UpdateBrandingSchema), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const currentBranding: any = existing.branding || {};
 ...
 */
router.patch('/info', requireAuth, requireRole('SCHOOL_ADMIN'), validate(UpdateSchoolInfoSchema), async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.patch('/id-template', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), brandingUpload.single('template'), async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.patch('/id-template/front', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), brandingUpload.single('template'), async (req: AuthRequest, res: Response) => {
  try {
    const templatePath = (req as any).uploadCategoryPath && req.file
      ? `${(req as any).uploadCategoryPath}/${req.file.filename}`
      : undefined;

    if (!templatePath) return res.status(400).json({ error: 'No front template file uploaded' });
 ...
 */
router.patch('/id-template/back', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), brandingUpload.single('template'), async (req: AuthRequest, res: Response) => {
  try {
    const templatePath = (req as any).uploadCategoryPath && req.file
      ? `${(req as any).uploadCategoryPath}/${req.file.filename}`
      : undefined;

    if (!templatePath) return res.status(400).json({ error: 'No back template file uploaded' });
 ...
 */
router.get('/id-card/students', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
    const { classId } = req.query;
    const students = await prisma.student.findMany({
      where: {
        schoolId: req.user!.schoolId!,
        ...(classId && classId !== 'all' ? { classId: classId as string } : {})
      },
      select: {
        id: true,
        name: true,
 ...
 */
router.get('/id-card/data/:studentId', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
    const schoolId = req.user!.schoolId!;
    ...
    const [school, settings, student] = await Promise.all([
      ...
      prisma.student.findFirst({
        where: { id: req.params.studentId as string, schoolId },
        select: {
          id: true,
          name: true,
          studentId: true,
    ...
    const cls = student.classId ? await prisma.schoolClass.findFirst({ where: { id: student.classId }, select: { name: true, level: true } }) : null;
    ...
    res.json({
      ...
      templates: {
        front: (settings as any)?.idCardTemplateFront || school.idCardTemplate || null,
        back: (settings as any)?.idCardTemplateBack || null,
      },
      student: {
        id: student.id,
        name: student.name,
 ...
 */
router.patch('/favicon', requireAuth, requireRole('SCHOOL_ADMIN'), brandingUpload.single('favicon'), async (req: AuthRequest, res: Response) => {
  try {
    const faviconPath = (req as any).uploadCategoryPath && req.file 
      ? `${(req as any).uploadCategoryPath}/${req.file.filename}` 
      : undefined;

    if (!faviconPath) return res.status(400).json({ error: 'No favicon file uploaded' });
...
// ── STUDENT HOUSES ──
router.get('/houses', requireAuth, async (req: AuthRequest, res) => {
  try {
...
router.post('/houses', requireAuth, requireRole('SCHOOL_ADMIN'), brandingUpload.single('logo'), validate(HouseSchema), async (req: AuthRequest, res) => {
  try {
...
router.patch('/houses/:id', requireAuth, requireRole('SCHOOL_ADMIN'), brandingUpload.single('logo'), validate(HouseSchema), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const { name, description, houseMasterId, houseCaptainId, color, motto } = req.body;
    const logoFilename = req.file ? req.file.filename : undefined;
    ...
    const house = await prisma.studentHouse.update({
      where: { id: id as string, schoolId: req.user!.schoolId! },
      data: updateData,
      include: {
        houseMaster: {
          select: {
...
router.delete('/houses/:id', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  try {
    await prisma.studentHouse.delete({ where: { id: req.params.id as string } });
...
router.post('/houses/assign-student', requireAuth, validate(AssignHouseStudentSchema), async (req: AuthRequest, res) => {
  try {
...
router.get('/houses/:id/members', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const members = await prisma.student.findMany({
      where: { houseId: id as string, schoolId: req.user!.schoolId as string },
      include: {
...
router.patch('/clubs-list/:id', requireAuth, requireRole('SCHOOL_ADMIN'), clubsUpload.single('logo'), validate(ClubSchema), async (req: AuthRequest, res) => {
  try {
    ...
    if (logoFilename) updateData.icon = logoFilename;
    if (category !== undefined) updateData.category = category || null;
    if (patron !== undefined) updateData.patron = patron || null;
    if (chairperson !== undefined) updateData.chairperson = chairperson || null;
...
// ── SCHOOL SPORTS ──
router.get('/sports-list', requireAuth, async (req: AuthRequest, res) => {
  try {
...
router.post('/sports-list', requireAuth, requireRole('SCHOOL_ADMIN'), sportsUpload.single('logo'), validate(SportSchema), async (req: AuthRequest, res) => {
  try {
    const { name, description, category, coach, sportMaster, captain, sportMasterId, captains, coaches, ageGroups } = req.body;
    const logoFilename = req.file ? req.file.filename : undefined;

    let ageGroupsArr: string[] = [];
    if (ageGroups) {
      if (typeof ageGroups === 'string') {
        try {
          ...
          if (Array.isArray(parsed)) {
            ...
            ageGroupsArr = ageGroups.split(',').map((s: string) => s.trim()).filter(Boolean);
          }
        } catch {
          ageGroupsArr = ageGroups.split(',').map((s: string) => s.trim()).filter(Boolean);
        }
      ...
      if (typeof captains === 'string') {
        try {
          ...
          if (Array.isArray(parsed)) {
            ...
            captainsArr = captains.split(',').map((s: string) => s.trim()).filter(Boolean);
          }
        } catch {
          captainsArr = captains.split(',').map((s: string) => s.trim()).filter(Boolean);
        }
...
router.patch('/sports-list/:id', requireAuth, requireRole('SCHOOL_ADMIN'), sportsUpload.single('logo'), validate(SportSchema), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const { name, description, category, coach, sportMaster, captain, sportMasterId, captains, coaches, ageGroups } = req.body;
    const logoFilename = req.file ? req.file.filename : undefined;
    ...
    if (sportMaster !== undefined) updateData.sportMaster = sportMaster || null;
    if (sportMasterId !== undefined) updateData.sportMasterId = sportMasterId || null;
    if (captain !== undefined) updateData.captain = captain || null;
...
router.delete('/sports-list/:id', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  try {
...
// ── SPORTING EQUIPMENT ──
router.get('/sports-equipment', requireAuth, async (req: AuthRequest, res) => {
  try {
...
router.post('/sports-equipment', requireAuth, requireRole('SCHOOL_ADMIN'), validate(SportEquipmentSchema), async (req: AuthRequest, res) => {
  try {
...
router.patch('/sports-equipment/:id', requireAuth, requireRole('SCHOOL_ADMIN'), validate(SportEquipmentSchema), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const { name, sportId, quantity, condition, custodianId } = req.body;
    ...
    if (custodianId !== undefined) updateData.custodianId = custodianId || null;
...
router.delete('/sports-equipment/:id', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
...
router.post('/holidays', requireAuth, requireRole('SCHOOL_ADMIN'), validate(HolidaySchema), async (req: AuthRequest, res) => {
  try {
...
router.delete('/holidays/:id', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  try {
 ...
 */
router.get('/tenant/export', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  try {
    const schoolId = req.user!.schoolId!;
    ...
    const users = await prisma.user.findMany({ where: { schoolId }, select: { id: true, name: true, email: true, role: true } });
    const fees = await prisma.fee.findMany({ where: { schoolId } });
    const payments = await prisma.studentPayment.findMany({ where: { schoolId } });
    const classes = await prisma.schoolClass.findMany({ where: { schoolId } });
    ...
    await logAction(req, 'EXPORT_TENANT_DATA', 'School', schoolId, {
      recordsExported: {
        users: users.length,
...
router.get('/:code', async (req, res) => {
  ...
  try {
    ...
    if (!school) return res.status(404).json({ error: 'School not found' });
    if (school.status === 'suspended') return res.status(403).json({ error: 'School access is suspended' });
    ...
    if (!websiteSettings) {
      ...
      (school as any).websiteSettings = websiteSettings;
    }
    ...
    if (!schoolSetting) {
      ...
      (school as any).schoolSetting = schoolSetting;
    }
 ...
 */
router.patch('/supplier-categories', requireAuth, requireRole('SCHOOL_ADMIN'), validate(SupplierCategoriesSchema), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const currentContent = (school.customContent as any) || {};
 ...
 */
router.patch('/:code', requireAuth, requireRole('SUPER_ADMIN'), validate(SuperAdminSchoolSchema), async (req: AuthRequest, res) => {
 ...
 */
router.get('/connections/pending', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const pendingSuppliers = pendingConnections.map(c => ({
      ...
      connectionId: c.id,
      metadata: {
        ...((c.supplier.user?.metadata as any) || {}),
 ...
 */
router.patch('/connections/:userId/approve', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res) => {
  try {
    const applicantId = req.params.userId as string;
    const school = await prisma.school.findUnique({ where: { id: req.user!.schoolId! } });
    if (!school) return res.status(404).json({ error: 'School not found' });

    const applicant = await prisma.user.findFirst({ where: { id: applicantId } });
    ...
    let connection: any = await prisma.schoolSupplier.findFirst({
        where: {
            schoolId: school.id,
