 ...
 */
router.get('/question-papers', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const flattenedPapers = papers.map(paper => ({
      ...
      teacher: {
        id: paper.teacher?.id,
        name: (paper.teacher as any)?.user?.name || 'Academic Admin'
      }
    }));
 ...
 */
router.post('/question-papers', requireAuth, requireRole('TEACHER', 'SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
    const schoolId = req.user!.schoolId!;
    ...
    const paper = await prisma.questionPaper.create({
      data: {
        ...validatedData,
        schoolId,
        teacherId: (teacher?.id || undefined) as any // Bypass out-of-sync types until generate is run
      }
    });
 ...
 */
router.put('/question-papers/:id', requireAuth, requireRole('TEACHER', 'SCHOOL_ADMIN'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const schoolId = req.user!.schoolId!;
    const validatedData = QuestionPaperSchema.parse(req.body);
 ...
 */
router.delete('/question-papers/:id', requireAuth, requireRole('TEACHER', 'SCHOOL_ADMIN'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const schoolId = req.user!.schoolId!;
    await prisma.questionPaper.deleteMany({ where: { id: id as string, schoolId } });
