 ...
 */
router.get('/summary', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/fees-balances', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const schoolId = req.user!.schoolId!;
  try {
 ...
 */
router.get('/payroll-runs', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.get('/payroll-run-details/:id', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.get('/employee-payslips', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
    const entries = await prisma.payrollEntry.findMany({
      where: { schoolId: req.user!.schoolId! },
 ...
 */
router.get('/tax-contributions', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  try {
    const schoolId = req.user!.schoolId!;
    const runs = await prisma.payrollRun.findMany({
      where: { schoolId },
 ...
 */
router.get('/grocery-consumption', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/profit-loss', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const { from, to } = req.query;
  const schoolId = req.user!.schoolId!;
 ...
 */
router.get('/detailed-expenses', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    const expenses = await prisma.expense.findMany({
      where: {
        ...
        ...(paymentMode ? { paymentMode: paymentMode as string } : {}),
        ...(from || to ? {
          date: {
            ...(from ? { gte: new Date(from as string) } : {}),
            ...(to ? { lte: new Date(to as string) } : {})
    ...
    // Grouping
    const byType = expenses.reduce((acc: any, e) => {
      acc[e.category.name] = (acc[e.category.name] || 0) + e.amount;
      return acc;
    }, {});

    const byMethod = expenses.reduce((acc: any, e) => {
 ...
 */
router.get('/revenue-allocation', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const { allocationId, classId } = req.query;
  const schoolId = req.user!.schoolId!;

  try {
    const allocation = await prisma.revenueAllocation.findFirst({
      where: { id: allocationId as string, schoolId },
      include: { feeGroups: true }
    });

    if (!allocation) return res.status(404).json({ error: 'Allocation config not found' });
    ...
    const breakdown = (allocation.breakdown as any[]).map(item => ({
      label: item.label,
 ...
 */
router.get('/enrollment-grouped', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const { groupName, studentCategory, classCategory, classIds } = req.query;
  const schoolId = req.user!.schoolId!;

  try {
    const selectedClasses = classIds ? (classIds as string).split(',') : [];

    const students = await prisma.student.findMany({
      where: {
        schoolId,
        ...(selectedClasses.length > 0 ? { classId: { in: selectedClasses } } : {}),
        ...(studentCategory && studentCategory !== 'All' ? { boardingStatus: studentCategory as string } : {}),
 ...
 */
router.get('/fees-takings', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    fees.forEach(f => {
      ...
      if (!perClass[className]) perClass[className] = { name: className, total: 0, studentCount: new Set() };
      perClass[className].total += f.paid;
      perClass[className].studentCount.add(f.studentId);

      const stuId = f.studentId;
      if (!perStudent[stuId]) perStudent[stuId] = { admissionNo: f.student.studentId, name: f.student.name, className, receipts: 0, total: 0 };
    ...
    res.json({
      ...
      byClass: Object.values(perClass).map((c: any) => ({ 
        name: c.name, 
        total: c.total, 
 ...
 */
router.get('/payments-analytics', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    const fees = await prisma.fee.findMany({
      where: {
        schoolId,
        paid: { gt: 0 },
        ...(feeGroupId ? { feeGroupId: feeGroupId as string } : {}),
        // simulate receipt search with id suffix
        ...(receiptNo ? { id: { endsWith: receiptNo as string } } : {}),
        ...(from || to ? {
          updatedAt: {
            ...(from ? { gte: new Date(from as string) } : {}),
            ...(to ? { lte: new Date(to as string) } : {})
    ...
    const byDay: any = {};
    const byWeek: any = {};
    const byMode: any = { 'ZB Bank': 0, 'Cash': 0 };
    const byGroup: any = {};
    ...
    const dateCounts = fees.reduce((acc: any, f) => {
      const key = f.updatedAt.getTime();
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    const duplicates = Object.values(dateCounts).filter((c: any) => c > 1).length;
 ...
 */
router.get('/student-debtors', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    const fees = await prisma.fee.findMany({
      where: {
        ...
        ...(term && term !== 'All Terms' ? { term: term as string } : {}),
        ...(status && status !== 'All Statuses' ? { student: { status: status as string } } : {})
      },
      include: {
        student: {
          include: { class: { select: { name: true } } }
 ...
 */
router.get('/balances-summary', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const { year, status, feeGroupIds } = req.query;
  const schoolId = req.user!.schoolId!;

  try {
    const selectedIds = feeGroupIds ? (feeGroupIds as string).split(',') : [];
    ...
    const detailedSummary = Object.values(groupStats).map((s: any) => ({
      name: s.name,
 ...
 */
router.get('/single-fee-group', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const { feeGroupId, classId, status } = req.query;
  const schoolId = req.user!.schoolId!;

  try {
    if (!feeGroupId) return res.status(400).json({ error: 'Fee Group ID is required' });
 ...
 */
router.get('/student-balances', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const { year, feeGroupIds, classIds, showBalanceOnly, searchName, searchSurname } = req.query;
  const schoolId = req.user!.schoolId!;

  try {
    const selectedFeeGroupIds = feeGroupIds ? (feeGroupIds as string).split(',') : [];
    const selectedClassIds = classIds ? (classIds as string).split(',') : [];
    ...
    const students = await prisma.student.findMany({
      where: {
        ...
        ...(searchName ? { name: { contains: searchName as string, mode: 'insensitive' } } : {}),
        // Simple search logic for surname if surname is part of name string
        ...(searchSurname ? { name: { contains: searchSurname as string, mode: 'insensitive' } } : {})
      },
      include: {
 ...
 */
router.get('/communication-logs', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
      ...
      where: {
        ...
        student: {
          OR: [
            ...
            { name: { contains: (searchSurname as string) || '', mode: 'insensitive' } }
          ]
        }
      },
      include: {
 ...
 */
router.get('/payment-history', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
    ...
    const students = await prisma.student.findMany({
      where: {
        ...
        OR: [
          ...
          { studentId: { contains: (searchTerm as string) || '', mode: 'insensitive' } }
        ]
      },
      include: {
        payments: {
 ...
 */
router.get('/uniforms-analytics', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const schoolId = req.user!.schoolId!;
 ...
 */
router.get('/fees-payments', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    const payments = await prisma.studentPayment.findMany({
      where: {
        schoolId,
        ...(from && to ? { date: { gte: new Date(from as string), lte: new Date(to as string) } } : {}),
        ...(paymentMode ? { paymentMode: paymentMode as string } : {}),
        ...(searchTerm ? {
          student: {
            OR: [
 ...
 */
router.get('/audit-logs', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/fee-reminders', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/institutional-overview', requireAuth, requireRole('SCHOOL_ADMIN', 'BURSAR'), async (req: AuthRequest, res: Response) => {
  const schoolId = req.user!.schoolId!;
