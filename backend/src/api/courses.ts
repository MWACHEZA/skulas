...
// Enroll a student
router.post('/enroll', requireAuth, requireRole('TEACHER', 'SCHOOL_ADMIN', 'SUPER_ADMIN'), async (req: AuthRequest, res) => {
  try {
...
router.get('/:courseId/enrollments', requireAuth, async (req: AuthRequest, res) => {
  try {
...
router.get('/revenue/report', requireAuth, requireRole('TEACHER'), async (req: AuthRequest, res) => {
  try {
    const courses = await prisma.course.findMany({
      where: {
        schoolId: req.user!.schoolId!,
