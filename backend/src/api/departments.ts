...
router.put('/:id', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Audit
    logSecurityEvent({ actorId: req.user!.id, action: 'UPDATE_DEPARTMENT', entityType: 'Department', entityId: id as string, details: { name }, schoolId: req.user!.schoolId!, ipAddress: req.ip });
    res.json(updated);
  } catch (error) {
...
router.delete('/:id', requireAuth, requireRole('SCHOOL_ADMIN'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Audit
    logSecurityEvent({ actorId: req.user!.id, action: 'DELETE_DEPARTMENT', entityType: 'Department', entityId: id as string, details: {}, schoolId: req.user!.schoolId!, ipAddress: req.ip });
