  ...
  try {
    ...
    const enhancedApplications = applications.map(app => {
      if (app.school?.type?.toLowerCase() === 'university' && app.academicData) {
        const data = app.academicData as any;
        const entryCategory = (app.entryCategory as any) || 'Normal';
        ...
        const eligibility = checkUniversityEligibility({
          entryCategory,
          oLevels: (data.oLevels || []).map((o: any) => o.subject).filter(Boolean),
          aLevels: (data.aLevels || []).map((a: any) => a.subject).filter(Boolean),
          age
        });
...
router.post('/', async (req: Request, res: Response) => {
  const { 
    applicantName, email, phone, dob, gender, appType, schoolCode,
    prevSchool, reasonForTransfer, lastGradeAchieved, academicHistory,
    address, notes, entryCategory, academicData,
    programLevel, studyMode, researchTitle
  } = req.body;

  try {
    const school = await prisma.school.findUnique({ where: { code: (schoolCode as string)?.toUpperCase() } });
    if (!school) return res.status(404).json({ error: 'School not found. Check school code.' });
...
router.get('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const application = await prisma.application.findFirst({
      where: { id: String(id), schoolId: req.user!.schoolId as string },
      include: { 
    ...
    if (application.school?.type?.toLowerCase() === 'university' && application.academicData) {
      const data = application.academicData as any;
      const entryCategory = (application.entryCategory as any) || 'Normal';
...
router.patch('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (status === 'accepted') {
        if (!classId) throw new Error('Class assignment is required for approval');
        ...
        if (!existingStudent) {
          ...
          if (level === 'PG_DIPLOMA') maxDate.setFullYear(maxDate.getFullYear() + Math.ceil(1 * multiplier));
          else if (level === 'MASTERS_TAUGHT' || level === 'MASTERS_RESEARCH') maxDate.setFullYear(maxDate.getFullYear() + Math.ceil(2 * multiplier));
          else if (level === 'MPHIL') maxDate.setFullYear(maxDate.getFullYear() + Math.ceil(3 * multiplier));
          else if (level === 'PHD') maxDate.setFullYear(maxDate.getFullYear() + Math.ceil(5 * multiplier));
          else maxDate.setFullYear(maxDate.getFullYear() + 6); // Max duration for UG (NUST Regs usually 6 years)
...
router.post('/documents', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!filename) return res.status(400).json({ error: 'Upload failed' });
 ...
 */
router.patch('/documents/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
