...
import { seedChartOfAccounts, getAccountId } from '../../prisma/seeders/coa.seeder';
...
async function jeExists(sourceType: string, sourceId: string): Promise<boolean> {
...
async function migrateStudentPayments(schoolId: string): Promise<number> {
  ...
  for (const payment of payments) {
    if (await jeExists('fee_payment', payment.id)) continue;

    try {
      const cashId = await getAccountId(schoolId, '1100', prisma);
      const arId = await getAccountId(schoolId, '1210', prisma);
  ...
  for (const fee of fees) {
    if (await jeExists('fee_invoice', fee.id)) continue;

    try {
      const arId = await getAccountId(schoolId, '1210', prisma);
      const incomeId = await getAccountId(schoolId, '5100', prisma);
...
async function migrateUniformSales(schoolId: string): Promise<number> {
  ...
  for (const sale of sales) {
    if (await jeExists('uniform_sale', sale.id)) continue;

    try {
      const cashId = await getAccountId(schoolId, '1100', prisma);
      const salesId = await getAccountId(schoolId, '5200', prisma);
      const cogsId = await getAccountId(schoolId, '6100', prisma);
      const invId = await getAccountId(schoolId, '1300', prisma);
      ...
      // COGS entry
      const totalCogs = sale.items.reduce(
        (s, i) => s + (i.item.costPrice || i.unitPrice * 0.7) * i.quantity,
        0
      );
      if (totalCogs > 0) {
        await LedgerService.postEntry({
      ...
      for (const saleItem of sale.items) {
        const movExists = await prisma.uniformStockMovement.findFirst({
...
async function migrateUniformPurchases(schoolId: string): Promise<number> {
  ...
  for (const order of orders) {
    if (await jeExists('uniform_purchase', order.id)) continue;

    try {
      const invId = await getAccountId(schoolId, '1300', prisma);
      const apId = await getAccountId(schoolId, '3110', prisma);
...
async function migratePayrollRuns(schoolId: string): Promise<number> {
  const runs = await prisma.payrollRun.findMany({ where: { schoolId } });

  log(`  PayrollRuns: ${runs.length} to migrate`);
  let count = 0;

  for (const run of runs) {
    if (await jeExists('payroll', run.id)) continue;

    try {
      const salId = await getAccountId(schoolId, '7100', prisma);
      const accruedId = await getAccountId(schoolId, '3300', prisma);
      const payeId = await getAccountId(schoolId, '3400', prisma);
...
async function migrateIncomeRecords(schoolId: string): Promise<number> {
  const incomes = await prisma.income.findMany({ where: { schoolId } });

  log(`  Income records: ${incomes.length} to migrate`);
  let count = 0;

  for (const income of incomes) {
    if (await jeExists('income', income.id)) continue;

    try {
      const cashId = await getAccountId(schoolId, '1100', prisma);
      const incomeId = await getAccountId(schoolId, '5900', prisma); // Miscellaneous Income (default)
...
async function migrateExpenseRecords(schoolId: string): Promise<number> {
  const expenses = await prisma.expense.findMany({ where: { schoolId } });

  log(`  Expense records: ${expenses.length} to migrate`);
  let count = 0;

  for (const expense of expenses) {
    if (await jeExists('expense', expense.id)) continue;

    try {
      const cashId = await getAccountId(schoolId, '1100', prisma);
      const expId = await getAccountId(schoolId, '7900', prisma); // Miscellaneous Expense (default)
...
async function main() {
  ...
  const schools = targetSchoolId
    ? await prisma.school.findMany({ where: { id: targetSchoolId } })
  ...
  for (const school of schools) {
    ...
    await seedChartOfAccounts(school.id, prisma);
