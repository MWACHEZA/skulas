 ...
 */
export function computeLoanFine(
  loan: { dueDate: Date; studentId?: string | null; waivedFine?: number | null; paidFine?: number | null },
  setting?: {
...
): { daysOverdue: number; fineAmount: number; isOverdue: boolean } {
  const now = new Date();
 ...
 */
export async function runLibraryReminders(): Promise<{ sentCount: number; errors: any[] }> {
  ...
  try {
    ...
    const settingsBySchool: Record<string, any> = {};

    for (const loan of activeLoans) {
      try {
        if (!settingsBySchool[loan.schoolId]) {
          settingsBySchool[loan.schoolId] = await prisma.librarySetting.findUnique({
...
export const libraryReminderWorker = new LibraryReminderWorker();
