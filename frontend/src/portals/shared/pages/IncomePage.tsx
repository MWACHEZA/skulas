...
const IncomePage: React.FC = () => {
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/accounts/categories', { name: newCategoryName.trim(), type: 'INCOME' });
      toast.success('Revenue category cataloged');
      setNewCategoryName('');
      fetchData();
    } catch (error: any) {
  ...
  const fetchData = async () => {
    try {
      ...
      setCategories((Array.isArray(catRes.data) ? catRes.data : []).filter((c: any) => c.type === 'INCOME'));
    } catch (error) {
    ...
    <div className="portal-container">
      <div className="portal-page-header">
        <div className="header-content">
          ...
          <p>Monitor and audit secondary institutional revenue streams and miscellaneous income inflows.</p>
        </div>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div className="status-badge" style={{ padding: '8px 20px', background: '#ecfdf5', color: '#059669', border: '1px solid #d1fae5', fontWeight: 900 }}>
            <i className="fas fa-balance-scale-left mr-2"></i>CREDIT LEDGER
          </div>
          <button 
          ...
          <button 
            ...
            style={{ padding: '12px 28px', fontWeight: 900, background: '#059669' }}
          >
            <i className="fas fa-plus-circle mr-2"></i>Record Institutional Revenue
          </button>
        </div>
      </div>
      ...
      <div className="management-table-card animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="portal-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '32px', borderBottom: '1px solid #f1f5f9' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: '#1e293b' }}>Institutional Revenue Audit Ledger</h3>
            <p style={{ margin: '4px 0 0 0', color: '#64748b', fontWeight: 700, fontSize: '0.9rem' }}>Comprehensive history of secondary revenue inflows.</p>
          </div>
          <span className="status-badge" style={{ fontWeight: 900, background: '#f8fafc', color: '#475569', border: '1px solid #f1f5f9', padding: '8px 16px' }}>
            {(Array.isArray(incomes) ? incomes : []).length} RECORDED ENTRIES
          </span>
        </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '100px' }}>
                    ...
                    <p style={{ fontWeight: 800, color: '#64748b' }}>Synchronizing ledger data...</p>
                  </td>
                </tr>
              ) : (Array.isArray(incomes) ? incomes : []).length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '100px', color: '#94a3b8' }}>
                    <i className="fas fa-hand-holding-usd" style={{ fontSize: '3.5rem', display: 'block', marginBottom: '24px', opacity: 0.1 }}></i>
                    <p style={{ fontWeight: 700, fontSize: '1.1rem' }}>No revenue transactions identified</p>
                  </td>
                </tr>
              ) : (Array.isArray(incomes) ? incomes : []).map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <span style={{ fontWeight: 900, color: '#059669' }}>{currencySymbol}{row.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </td>
                  <td>
            ...
            <form onSubmit={handleSubmit}>
              <div className="portal-modal-body" style={{ padding: '40px' }}>
                <div className="form-group" style={{ marginBottom: '24px' }}>
                  <label className="portal-label">Income Source Description</label>
                  <input 
                    type="text" 
                    className="portal-input"
                    placeholder="e.g. Canteen Operations - Weekly Sales"
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
                  <div className="form-group">
                    <label className="portal-label">Receipt Date</label>
                    <input 
                      type="date" 
                      className="portal-input"
                      value={formData.date}
                  ...
                  <div className="form-group">
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input 
                        ...
                        style={{ paddingLeft: `${(currencySymbol.length * 8) + 24}px`, fontWeight: 900, color: '#059669', height: '56px', fontSize: '1.1rem' }}
                        required
                      />
                    </div>
                  </div>
                ...
                <div className="form-group" style={{ marginBottom: '24px' }}>
                  <label className="portal-label">Revenue Category</label>
                  <select 
                    className="portal-input"
                    ...
                    required
                  >
                    <option value="">-- Select Strategic Category --</option>
        ...
        <div className="portal-modal-overlay">
          <div className="portal-modal-card animate-in zoom-in duration-200" style={{ maxWidth: '520px', padding: 0 }}>
            <div className="portal-modal-header" style={{ padding: '24px 32px', borderBottom: '1px solid #f1f5f9' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: '#1e293b' }}>Manage Revenue Classifications</h3>
                <p style={{ margin: '4px 0 0 0', color: '#64748b', fontWeight: 700, fontSize: '0.9rem' }}>Configure classification groups for institutional revenue streams.</p>
              </div>
            ...
            <div className="portal-modal-body" style={{ padding: '32px' }}>
              <form onSubmit={handleCreateCategory} style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
                ...
                <button 
                  type="submit"
                  className="portal-btn-primary"
                  style={{ padding: '0 24px', whiteSpace: 'nowrap', fontWeight: 900, background: '#059669', border: '1px solid #059669' }}
