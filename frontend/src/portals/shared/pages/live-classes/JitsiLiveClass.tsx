...
export default function JitsiLiveClass() {
  const { user } = useAuth();
  ...
  const fetchLiveClasses = async () => {
    try {
      const res = await api.get('/api/live-class?platform=Jitsi');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast('Jitsi Live Class Created Successfully', 'success');
      fetchLiveClasses();
      setFormData({
        ...
        <div className="portal-modal-backdrop" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', inset: 0, backgroundColor: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(8px)', zIndex: 1000, padding: 20 }}>
          <div className="portal-modal-content" style={{ background: 'white', borderRadius: 16, maxWidth: 550, width: '100%', padding: '30px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', position: 'relative' }}>
            ...
            <h2 style={{ margin: '0 0 20px 0', fontSize: '1.3rem', fontWeight: 800, color: '#0f172a' }}>
              <i className="fas fa-video mr-2" style={{ color: 'var(--portal-success)' }}></i> CREATE JITSI LIVE CLASS
            </h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
              <div className="portal-form-group">
                <label>Jitsi Meeting Link <span style={{ color: 'red' }}>*</span></label>
                <input type="text" className="portal-input" placeholder="Jitsi Room Link" value={formData.meetingId} onChange={e => setFormData({ ...formData, meetingId: e.target.value })} />
              </div>
              <div className="portal-form-group">
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }} className="no-print">
            <button 
              onClick={() => {
                ...
                exportToCSV('Jitsi_Live_Classes', headers, rows);
              }}
            ...
            <button 
              onClick={() => {
                ...
                exportToWord('Jitsi_Live_Classes', headers, rows);
              }}
              className="portal-btn-secondary"
              style={{ padding: '8px 16px', fontSize: '0.85rem' }}
