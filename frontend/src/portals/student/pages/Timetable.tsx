...
// Shared Period Generator (Match Admin)
...
export default function StudentTimetable() {
  ...
  const fetchTimetable = async () => {
    try {
      ...
      const studentId = isParent ? activeEntity?.id : undefined;
  ...
  return (
    <div className="portal-container">
      <div className="portal-page-header">
        <h1>{user?.role === 'PARENT' ? `${activeEntity?.name}'s Timetable` : 'My School Timetable'}</h1>
        <p>{user?.role === 'PARENT' ? `Weekly schedule for ${activeEntity?.name}.` : 'Your weekly academic schedule and assigned teachers.'}</p>
      </div>

      <div className="portal-card">
        <div className="portal-card-header">
           <h2><i className="fas fa-calendar-alt" style={{ marginRight: 8, color: 'var(--portal-primary)' }}></i>Weekly View</h2>
           <button onClick={() => window.print()} className="portal-btn-secondary">
             <i className="fas fa-print" style={{ marginRight: 6 }}></i>Print Timetable
           </button>
        </div>
        <div className="portal-card-body" style={{ padding: 0, overflowX: 'auto' }}>
          {loading ? (
            ...
            <div style={{ padding: 60, textAlign: 'center' }}>
              <i className="fas fa-clock" style={{ fontSize: '3rem', color: '#cbd5e0', marginBottom: 20 }}></i>
              <h3>Timetable Pending</h3>
              <p>Your official timetable hasn't been published by the admin yet.</p>
            </div>
          ) : (
            <table className="timetable-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={{ background: '#f8fafc', padding: '12px', border: '1px solid #e2e8f0' }}>Period</th>
                ...
                {periods.map((p, pIdx) => (
                  <tr key={pIdx} className={p.type !== 'lesson' ? 'break-row' : ''}>
                    ...
                    {DAYS.map((_, dIdx) => {
                       if (p.type !== 'lesson') {
                         return (
                            <td key={dIdx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', textAlign: 'center', color: '#94a3b8', fontStyle: 'italic', fontSize: '0.8rem' }}>
                              {p.label}
                            </td>
                         );
                       }
                           ...
                           {slot ? (
                             <>
                               ...
                               <div style={{ fontSize: '0.75rem', color: '#718096', marginTop: 4 }}>
                                 <i className="fas fa-map-marker-alt" style={{ marginRight: 4 }}></i>{slot.room || 'TBA'}
                               </div>
                             </>
                           ) : (
